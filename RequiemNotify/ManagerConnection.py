...
from RequiemEasy import ClientEasy, Connection, RequiemError
from RequiemNotify import ErrorDialog
...
class SingleConnection:
        ...
        def doConnect(self):
                ...
                self.env.notify.run(None, None, "Manager Connection successfull", "With Requiem-Manager <b>%s</b>" % self._con.GetPeerAddr())
        ...
        def handleDisconnect(self, err=""):
                if err:
                        err = ": " + err
                self.env.notify.run("high", None, "Manager Connection interrupted", "With Requiem-Manager <b>%s</b>%s" % (self._con.GetPeerAddr(), err))
...
class Session:
        def __init__(self, env, profile):
                self.env = env
                self.client = RequiemEasy.ClientEasy(profile, RequiemEasy.Client.IDMEF_READ)
