...
#
# This file is part of the Requiem-Notify program.
#
...
def ConfutatisURL(conf, idlist):
        if len(idlist) > 1:
                ...
                url = "%s?view=alert_listing%s" % (conf.get("confutatis", "url"), mid)
        else:
                url = "%s?view=alert_summary&origin=alert_listing&messageid=%s" % (conf.get("confutatis", "url"), idlist[0])
...
class NotifyNow:
        ...
        def _confutatis_view_cb(self, n, action, messageid):
                self._pynotify_bug.remove(n)

                if self._env.config.get("ui", "browser") == "auto":
                        webbrowser.open(ConfutatisURL(self._env.config, messageid), autoraise=True)
        ...
        def run(self, severity, messageid, title, message):
                ...
                if messageid and self._env.config.get("confutatis", "url"):
                        n.data = messageid
                        self._pynotify_bug.append(n)

                        n.add_action("response", "Confutatis View", self._confutatis_view_cb, messageid)
...
class RequiemNotify(NotifyQueue, NotifyNow):
        def __init__(self, env):
