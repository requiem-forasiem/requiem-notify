...
from RequiemNotify import siteconfig
from ConfigParser import SafeConfigParser

userconfigpath = "%s/.requiem-notify" % (os.getenv("HOME"))
globalthemespath = siteconfig.prefix + "/share/requiem-notify/themes"
userthemespath = userconfigpath + "/themes"

class PnConfig(SafeConfigParser):
        _globalconfigfile = siteconfig.conf_dir + "/requiem-notify.conf"
        _userconfigfile = userconfigpath + "/requiem-notifyrc"
        ...
        def _update(self):
                ...
                if self._updated.has_key("filter") :
                        if self._updated["filter"] != "":
                                try:
                                        self.env.criteria = RequiemEasy.IDMEFCriteria(self._updated["filter"][3])
                                except RequiemEasy.RequiemError, e:
